...
// Defining a Null Task:
typedef void(*task_T)(void);
#define	NULL (void*)0

#define BUS_FREQ		16000000
//#define	PERIOD_OF_TICK	100
...
static	periodicTask_T	PeriodicTasks[NUM_PERIODIC_TASKS];
...
uint8_t osKermelAddPeriodicThreads(void(*thread_1)(void),
																uint32_t period_1,
																void(*thread_2)(void),
																uint32_t period_2
															 )
{
	MaxPeriod		=		period_1 > period_2 ? period_1 : period_2;
	uint32_t		minPeriod		=		period_1 < period_2 ? period_1 : period_2;
	
	// CAUTION: Periods should be multiples of each other, for this to work properly:
	// In case it isn't, simply return from the function:
	if (MaxPeriod % minPeriod != 0)
		return 0;
	// Else, add periodic threads:
	PeriodicTasks[0].task		=		thread_1;
	PeriodicTasks[0].period	=		period_1;
	PeriodicTasks[1].task		=		thread_2;
	PeriodicTasks[1].period	=		period_2;
...
void osSchedularRoundRobin(void)
{
	// If a Time-out has occurred in our scheduler, we will first run our periodic task, and then move on to anything else:
	if (TimeMsec < MaxPeriod)	
		TimeMsec++;
	else
		TimeMsec = 1;
	// Checking for any periodic task that is ready to be executed:
	int i;
	...
	// Now moving on to our Round-Robin Scheduler:
