...
{
	SysTick->VAL = 0x0;																													// Resetting the SysTick Count value
	INTCTRL = 0x04000000;																												// Triggering SysTick Exception by setting the bit 26 of INTCTRL register
}
...
void osSignnalSet(int32_t *semaphore)
{
	__disable_irq();
	*semaphore += 1;
	__enable_irq();
