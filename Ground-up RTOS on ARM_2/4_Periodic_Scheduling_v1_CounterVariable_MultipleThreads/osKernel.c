...
{
	periodic_tick++;
	if (periodic_tick%PERIOD_OF_TICK == 1)
	{
		// Run the periodic task 1:
		(*periodicTask_1)();
	}
	
	if (periodic_tick%(2 * PERIOD_OF_TICK) == 1)
	{
		// Run the periodic task 1:
		(*periodicTask_2)();
	}
	
	// The very implementation that we commented out in our assembley code:
